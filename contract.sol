...
contract PROFIContract {
    ...
    mapping(address user => uint) balances;
    mapping(string code => address) public refCodesOwners;
    mapping(address user => bool) useCodes;
    mapping(address owner => string) public refCodes;
    mapping(address user => uint) discount;
    ...
    constructor(string memory _name, string memory _symbol, uint _totalSuply, uint _digits, address tom, address max, address jack) {
        owner = msg.sender;
        name = _name;
        symbol = _symbol;
